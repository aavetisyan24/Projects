...
class Futures1(Thread):
    ...
    def on_message(self, ws, message):
        ...
        list_info_usdt = [data['s'], data['a'], data['A'], "|||||||", data['b'], data['B']]
...
class Futures2(Thread):
    ...
    def on_message(self, ws, message):
        ...
        data = msg
        # runtime = self.unix_to_datetime(str(data['E']))
        list_info_busd = [data['s'], data['a'], data['A'], "|||||||", data['b'], data['B']]
...
def order_market(SIDE, symbol, PRICE, quantity=40):
    order_info = []
...
if __name__=="__main__":
    ...
    Bot2 = Futures2(f"wss://stream.binance.com:9443/ws/{coin}usdt@bookTicker")
    (Bot1.start())
    (Bot2.start())
    while DO_IT:
        if list_info_usdt and list_info_busd:
            ...
            usdt_ask, usdt_bid = float(list_info_usdt[1]), float(list_info_usdt[4])
            busd_ask, busd_bid = float(list_info_busd[1]), float(list_info_busd[4])
            if usdt_bid > busd_ask+(busd_ask*0.0004):# or busd_ask < usdt_bid-(usdt_bid*0.0003):
                print("Pair created")
                print("USDT-ask-", usdt_ask, "-bid-", usdt_bid)
                print("BUSD-bid-", busd_bid, "-ask-", busd_ask)
                # order_market(SIDE_SELL, "DOGEBUSD", PRICE=busd_bid, quantity=40)
                # order_market(SIDE_BUY, "DOGEUSDT", PRICE=usdt_ask, quantity=40)
    ...
    #     # print("1", (list_info_usdt))
    #     # print("2", (list_info_busd))
    #     usdt_bid = float(list_info_usdt[4])
    #     busd_ask = float(list_info_busd[1])
    #     if usdt_bid >= busd_ask+(busd_ask*0.0001):
    #         order_market(SIDE_BUY, "DOGEBUSD", PRICE=float(busd_ask), quantity=40)
    #         order_market(SIDE_SELL, "DOGEUSDT", PRICE=float(usdt_bid), quantity=40)
